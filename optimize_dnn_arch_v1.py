...
# Load the dataset
data = pd.read_csv(f'data/test_dataset.csv')
...
joblib.dump(scaler, f'weights/best_dnn_scaler_{version}.pkl')
...
def objective(trial):
    ...
    if np.any(np.isnan(y_pred)):
        return float('inf')  # Return a large error value to indicate failure
...
best_model.save(f'weights/best_dnn_model_{version}.h5')

# Save the best hyperparameters
with open(f'best_dnn_hyperparameters_{version}.json', 'w') as json_file:
...
plt.savefig(f'training_validation_loss_{version}.pdf')
plt.close()
